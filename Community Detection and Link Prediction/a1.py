...
def bfs(graph, root, max_depth):
    ...
    while deque_graph:
        current_node = deque_graph.popleft()
        traversal_level = level[current_node]
        
        if traversal_level < max_depth:
            for neighbour in graph.neighbors(current_node):            
                if neighbour not in visited:
                    level[neighbour] = node2distances[neighbour] = 1 + node2distances[current_node]
                    visited.append(neighbour)
                    deque_graph.append(neighbour)
        
                if level[neighbour] > level[current_node]:
                    node2parents[neighbour].append(current_node)                    
        else:
            continue
    ...
    for node in visited[1:]:    
        parent_list = node2parents[node]
        for parent in parent_list:
            if parent == root:
                node2num_paths[node] = 1
            else:
                node2num_paths[node] += node2num_paths[parent]
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    """
    credit_per_node = defaultdict(lambda:0)
    result = {}
    
    #create a dict of nodes bottom up
    bottom_up_list = sorted(node2distances, key=node2distances.get, reverse=True)
    
    node_list = list(node2parents.keys())
    node_list.append(root)
    
    child_nodes = defaultdict(lambda:[])
    denominator = 0.0
    
    for node in bottom_up_list:
        if node in node2parents.keys():
            parent_list = node2parents[node]
            for parent in parent_list:
                child_nodes[parent].append(node)
            
    leaf_node_list = []
    leaf_node_list = node2parents.keys() - child_nodes.keys()
    
    for leaf_node in leaf_node_list:
        credit_per_node[leaf_node] = 1
    
    for node in bottom_up_list:          
        if node not in leaf_node_list:
            value = 0.0
            for child_node in child_nodes[node]:
                if len(node2parents[child_node]) > 1:
                    denominator = 0
                    for parent in node2parents[child_node]:
                        denominator += node2num_paths[parent]
                        value = node2num_paths[parent]/denominator
                else:
                    value = credit_per_node[child_node]
                    
                credit_per_node[node] += value
                result[(tuple(sorted((node,child_node))))] = float(value)
                
            credit_per_node[node] += 1
...
def approximate_betweenness(graph, max_depth):
    ...
    """
    summation_result = defaultdict(lambda:0)
    for root in graph.nodes():
        node2distances,node2num_paths,node2parents = bfs(graph,root,max_depth)
        result = bottom_up(root,node2distances,node2num_paths,node2parents)
        for key in list(result.keys()):
            summation_result[key] += result[key]
                 
    summation_result.update((key,value/2) for key,value in summation_result.items())
...
def partition_girvan_newman(graph, max_depth):
    ...
    """
    components = []
    betweenness_graph_value = approximate_betweenness(graph, max_depth)
    betweenness_graph_list = sorted(betweenness_graph_value.items(), key = lambda element:(-(element[1]),element[0]))
    
    new_graph = graph.copy()
    
    while len(components) <= 1:
        edge_highest_betweenness = max(betweenness_graph_list,key = lambda element:element[1])
        edge_to_remove = list(edge_highest_betweenness[0])
        new_graph.remove_edge(edge_to_remove[0],edge_to_remove[1])
        betweenness_graph_list.remove(edge_highest_betweenness)
...
def score_max_depths(graph, max_depths):
    ...
    for depth in max_depths:
        ...
        result.append((depth,norm_cut_value))
...
def make_training_graph(graph, test_node, n):
    ...
    nodes_list = sorted(graph_copy.neighbors(test_node))
    nodes_list = nodes_list[:n]
    for node in nodes_list:
        graph_copy.remove_edge(test_node,node)
...
def jaccard(graph, node, k):
    ...
    for node_value in graph.nodes():
        if node_value != node and node not in graph.neighbors(node_value):
            neighbours2 = set(graph.neighbors(node_value))
            scores.append(((node,node_value), 1. * len(neighbours & neighbours2) / len(neighbours | neighbours2)))
    
    
    final_result = sorted(scores, key=lambda x: (-x[1],x[0]))
...
def path_score(graph, root, k, beta):
    ...
    list_of_nodes = list(set(graph.nodes()) - set(graph.neighbors(root)))
    list_of_nodes.remove(root)
    for node in list_of_nodes:
        beta_power_i = math.pow(beta,node2distances[node])
        n_x_y = node2num_paths[node]
        path_score = beta_power_i * n_x_y
        scores.append(((root,node),path_score))
        
    scores = sorted(scores,key = lambda element:(-element[1],element[0]))  
    return scores[:k]   
    pass
...
def evaluate(predicted_edges, graph):
    ...
    for edge in predicted_edges:
        if edge[1] in graph.neighbors(edge[0]):
            count += 1
            
    result = float(count/len(predicted_edges))
    return result
    pass
